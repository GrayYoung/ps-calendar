...
PSConstants.add = function(kind, name) {
	if (!name) {
		throw "Internal error PSConstants.add()";
	}
	kind._name = name;
	...
	kind._add = function(name, sym) {
		if (!name || !sym) {
			throw "Internal error kind._add()";
		}
...
PSString._add = function(name, sym) {
	if (!name) {
		throw "Internal error PSString._add()";
	}
	if (!sym)
		sym = name;
	var kind = this;
...
PSConstants.test = function() {
	...
	print('reverse of ' + PSClass.Action + " = " + PSConstants.reverseNameLookup(PSClass.Action));
